...
{elseif $hp && $email}
{assign var=email_hp value=" <a href=\"$hp\"><img src=\"$THEMES_DIR/$theme/images/general-homepage.svg\" title=\"$homepage_alt\" alt=\"⌂\" width=\"13\" height=\"13\" /></a> <a href=\"index.php?mode=contact&amp;id=$email_id\" rel=\"nofollow\"><img src=\"$THEMES_DIR/$theme/images/e-mail-envelope.svg\" title=\"$email_alt\" alt=\"@\" width=\"13\" height=\"13\" /></a>"}
{else}
...
<div class="reply">{if $data.$element.locked==0}<a class="boldlink" href="index.php?mode=posting&amp;id={$data.$element.id}&amp;back=thread" title="{#reply_link_title#}"><img class="icon wd-dependent" src="{$FORUM_ADDRESS}/{$THEMES_DIR}/{$theme}/images/arrow-bold-horizontal.svg" alt="" width="13" height="13"/><span>{#reply_link#}</span></a>{else}<span class="locked"><img class="icon" src="{$FORUM_ADDRESS}/{$THEMES_DIR}/{$theme}/images/general-lock-closed.svg" alt="" width="13" height="13"/><span>{#posting_locked#}</span></span>{/if}</div>
...
{if $data.$element.options.edit}<li><a href="index.php?mode=posting&amp;edit={$data.$element.id}&amp;back=thread" title="{#edit_message_linktitle#}"><img class="icon" src="{$FORUM_ADDRESS}/{$THEMES_DIR}/{$theme}/images/posting-edit.svg" alt="" width="13" height="13"/><span>{#edit_message_linkname#}</span></a></li>{/if}
{if $data.$element.options.delete}<li><a href="index.php?mode=posting&amp;delete_posting={$data.$element.id}&amp;csrf_token={$CSRF_TOKEN}&amp;back=thread" title="{#delete_message_linktitle#}"><img class="icon" src="{$FORUM_ADDRESS}/{$THEMES_DIR}/{$theme}/images/delete-cross.svg" alt="" width="13" height="13"/><span>{#delete_message_linkname#}</span></a></li>{/if}
{if $data.$element.options.move}<li><a href="index.php?mode=posting&amp;move_posting={$data.$element.id}&amp;back=thread" title="{#move_posting_linktitle#}"><img class="icon wd-dependent" src="{$FORUM_ADDRESS}/{$THEMES_DIR}/{$theme}/images/arrow-move.svg" alt="" width="13" height="13"/><span>{#move_posting_linkname#}</span></a></li>{/if}
{if $data.$element.options.report_spam}<li><a href="index.php?mode=posting&amp;report_spam={$data.$element.id}&amp;back=thread" title="{#report_spam_linktitle#}"><img class="icon" src="{$FORUM_ADDRESS}/{$THEMES_DIR}/{$theme}/images/general-caution.svg" alt="" width="13" height="13"/><span>{#report_spam_linkname#}</span></a></li>{/if}
...
<li><a href="index.php?mode=posting&amp;unlock_thread={$data.$element.id}&amp;back=thread" title="{#unlock_thread_linktitle#}"><img class="icon" src="{$FORUM_ADDRESS}/{$THEMES_DIR}/{$theme}/images/thread-unlock.svg" alt="" width="13" height="13"/><span>{#unlock_thread_linkname#}</span></a></li>
{else}
<li><a href="index.php?mode=posting&amp;lock_thread={$data.$element.id}&amp;back=thread" title="{#lock_thread_linktitle#}"><img class="icon" src="{$FORUM_ADDRESS}/{$THEMES_DIR}/{$theme}/images/thread-lock.svg" alt="" width="13" height="13"/><span>{#lock_thread_linkname#}</span></a></li>
{/if}
{/if}
{/if}
</ul>
