	...
	switch($action) {
		...
		case 'reorder':
			if (isset($_POST['bookmarks'])) {
				...
				$order_id = 1;
				foreach ($items as $id) {
					mysqli_query($connid, "UPDATE ".$db_settings['bookmark_table']." SET `order_id` = ". intval($order_id) ." WHERE id = ". intval($id) ." AND `user_id` = ". intval($user_id) ." LIMIT 1") or raise_error('database_error', mysqli_error($connid));
					$order_id++;
				}
			}
			exit;
