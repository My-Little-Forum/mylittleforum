...
if (isset($_SESSION[$settings['session_prefix'].'user_id']) || $settings['user_area_public'] == 1) {
	...
	if (isset($_POST['remove_account'])) $action = 'remove_account'; // delete, if GET ist used
	...
	switch($action) {
		...
		case 'remove_account':
			if (isset($_SESSION[$settings['session_prefix'].'user_id'])) {
				$id = $_SESSION[$settings['session_prefix'].'user_id'];
				$result = mysqli_query($connid, "SELECT `user_name` FROM `".$db_settings['userdata_table']."` WHERE `user_id` = ". intval($id) ." LIMIT 1") or raise_error('database_error', mysqli_error($connid));
				if (mysqli_num_rows($result) == 1) {
					$row = mysqli_fetch_array($result);
					mysqli_free_result($result);
					$smarty->assign('user_name', htmlspecialchars($row['user_name']));
