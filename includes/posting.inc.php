			...
			if (empty($errors)) {
				...
				if (isset($_POST['save_entry']) && $posting_mode == 0) {
					...
					if (intval($email_notification) === 1 and intval($newID) > 0) {
						@mysqli_query($connid, "INSERT INTO " .$db_settings['subscriptions_table']. " (user_id, start_id, hash) VALUES (" . intval($user_id) . ", " . intval($newID) . ", UUID())") or raise_error('database_error', mysqli_error($connid));
					}
					if ($id == 0) {
